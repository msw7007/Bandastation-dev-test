...
/// serpentid TAIL MARKINGS

/datum/bodypart_overlay/mutant/serpentid_tail_markings
...
/datum/bodypart_overlay/mutant/serpentid_tail_markings/get_global_feature_list()
	return SSaccessories.serpentid_tail_markings_list

/datum/bodypart_overlay/mutant/serpentid_tail_markings/get_base_icon_state()
	return (!isnull(tail_markings_key) ? "[tail_markings_key]_" : "") + (wagging ? "wagging_" : "") + sprite_datum.icon_state

/datum/bodypart_overlay/mutant/serpentid_tail_markings/can_draw_on_bodypart(mob/living/carbon/human/human)
	. = ..()
	if(human.wear_suit && (human.wear_suit.flags_inv & HIDEJUMPSUIT))
		return FALSE

/datum/bodypart_overlay/mutant/serpentid_tail_markings/override_color(rgb_value)
	return tail_markings_color

/obj/item/organ/tail/serpentid/on_bodypart_insert(obj/item/bodypart/bodypart)
	var/data = ..()
	insert_tail_markings(bodypart)
	return data

/obj/item/organ/tail/serpentid/on_bodypart_remove(obj/item/bodypart/bodypart)
	remove_tail_markings(bodypart)
	return ..()

/obj/item/organ/tail/serpentid/proc/insert_tail_markings(obj/item/bodypart/bodypart)
	if(tail_markings_overlay)
		return

	var/datum/sprite_accessory/tails/serpentid/tail_sprite_datum = bodypart_overlay.sprite_datum
...
/obj/item/organ/tail/serpentid/proc/remove_tail_markings(obj/item/bodypart/bodypart)
	if(!tail_markings_overlay)
		return
	bodypart.remove_bodypart_overlay(tail_markings_overlay)
	QDEL_NULL(tail_markings_overlay)

/obj/item/organ/tail/serpentid/start_wag(mob/living/carbon/organ_owner, stop_after = INFINITY)
	if(tail_markings_overlay)
		tail_markings_overlay.wagging = TRUE
	return ..()

/obj/item/organ/tail/serpentid/stop_wag(mob/living/carbon/organ_owner)
	if(tail_markings_overlay)
